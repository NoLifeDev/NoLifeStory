...
namespace nl {
    namespace music {
        ...
        int callback(const void *, void * output, unsigned long frames, PaStreamCallbackTimeInfo const *, PaStreamCallbackFlags, void * user) {
            ...
            while (todo) {
                ...
                if (err == MPG123_NEED_MORE) {
                    mpg123_feed(stream->handle, reinterpret_cast<unsigned char const *>(stream->a.data()) + 82, stream->a.length() - 82);
                }
            }
            if (stream->stop == true) {
                stream->stop = false;
        ...
        void play(node nn) {
            ...
            mpg123_feed(active->handle, reinterpret_cast<unsigned char const *>(a.data()) + 82, a.length() - 82);
            long rate;
            int encoding;
