...
physics::physics() { left = false, right = false, up = false, down = false; }
...
void physics::jump() {
    ...
    if (fh) {
        ...
        if (flying) { vy = -shoe_swim_speed_v * swim_jump; }
    }
}
void physics::update() {
    ...
    while (time::delta_total > laststep) {
        ...
        if (lr) {
            ...
            if (flying) {
                ...
                vx = vx < -vmax
                         ? std::min(-vmax, vx + shoefloat)
                         : vx > vmax ? std::max(vmax, vx - shoefloat)
                                     : mleft ? std::max(-vmax, vx - shoefloat)
                                             : mright ? std::min(vmax, vx + shoefloat)
